...
Result db[2003];
vector<int> taxiList[1003][1003]; // 택시번호
set<Data> tot; // 운행거리
...
void init(int N, int M, int L, int mXs[], int mYs[]) {
	::N = N, ::L = L, tot.clear();
	for (int i = 0; i < 1000; i++) for (int j = 0; j < 1000; j++) taxiList[i][j].clear();
	for (int i = 1; i <= M; i++) {
		db[i] = { mXs[i - 1], mYs[i - 1] };
		taxiList[mYs[i - 1] / 10][mXs[i - 1] / 10].push_back(i);
...
int pickup(int mSX, int mSY, int mEX, int mEY) {
	...
	int sx = max(mSX / 10 - L / 10, 0), sy = max(mSY / 10 - L / 10, 0);
	int ex = min(mSX / 10 + L / 10, N / 10), ey = min(mSY / 10 + L / 10, N / 10);
	...
	if (!tmp.empty()) {
		...
		// 택시 이동 처리
		auto& t = taxiList[d.mY / 10][d.mX / 10];
		for (int k = 0; k < t.size(); k++) {
			...
			d1.mX = mEX, d1.mY = mEY;
			taxiList[d1.mY / 10][d1.mX / 10].push_back(taxi_number);
