...
set<Data> ver[MAXN], ld[MAXN], hor[MAXN], rd[MAXN];
set<int> v[1003];
set<Data>::iterator it;
void init() {
	idCnt = 0, idMap.clear();
	for (int i = 0; i < MAXN; i++)
		ver[i].clear(), hor[i].clear(), ld[i].clear(), rd[i].clear();
...
void contactTracing(int uID, int visitNum, int moveInfo[], int visitList[]) {
	...
	for (int i = 1; i < visitNum; i++) {
		...
		if (d == 0) { // 0번방향
			...
			it = lower_bound(rd[r - c + 10000].begin(), rd[r - c + 10000].end(), Data{ pid });
			while (it != rd[r - c + 10000].end() && !flag) update(uID, it, BACK, visitList);
		}
		else if (d == 4) {// 4번방향
			it = lower_bound(ver[c].begin(), ver[c].end(), Data{ pid });
			...
			it = lower_bound(rd[r - c + 10000].begin(), rd[r - c + 10000].end(), Data{ pid });
			while (it != rd[r - c + 10000].begin() && !flag) update(uID, it, FRONT, visitList);
		}
	}
}
