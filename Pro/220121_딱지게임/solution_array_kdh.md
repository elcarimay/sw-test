...
int addDdakji(int mRow, int mCol, int mSize, int mPlayer) {
	...
	for (int r = mRow / M; r <= (mRow + mSize - 1) / M; r++) for (int c = mCol / M; c <= (mCol + mSize - 1) / M; c++) {
		int n = mn[r][c];
		for(int i = 0;i < mn[r][c];i++)
			if (overlap(m[r][c][i], idCnt)) unionSet(m[r][c][i], idCnt);
		m[r][c][mn[r][c]++] = idCnt;
	}
	idCnt++;
	return pCnt[mPlayer];
...
int check(int mRow, int mCol) {
	int n = mn[mRow / M][mCol / M];
	for (int i = 0; i < mn[mRow / M][mCol / M]; i++) {
		int nid = m[mRow / M][mCol / M][i];
