...
struct Worm {
    ...
    void placeStraight(int x, int y, int L, int face, int N_) {
        ...
        int bdir = (face + 2) % 4; // tail쪽 방향
        runs.clear();
        runs.push_front((Run){face, 1});
        if (L - 1 > 0) runs.push_back((Run){bdir, L - 1});
        static int dx[4] = {0, 1, 0, -1};
        static int dy[4] = {-1, 0, 1, 0};
        tx = x + dx[bdir] * (L - 1);
        ty = y + dy[bdir] * (L - 1);
    ...
    pair<int,int> previewTailPopCell() const {
        if (runs.empty()) return make_pair(tx, ty);
        int tdir = runs.back().dir;
        static int dx[4] = {0, 1, 0, -1};
        static int dy[4] = {-1, 0, 1, 0};
        return make_pair(tx + dx[tdir], ty + dy[tdir]);
    }
...
static unordered_map<long long,int> id2idx;
...
void advanceTo(int t){
    ...
    for (; curTime < t; ++curTime){
        ...
        for(int i=0;i<W;i++) if(considered[i] && !dead[i]){
            if(!willGrow[i]){
                pair<int,int> p = worms[i].previewTailPopCell();
