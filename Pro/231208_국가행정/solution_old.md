...
void init(int N, int mPopulation[]){
	...
	for (int i = 0; i < N; i++) {
		cities[i] = 0; roads[i] = {};
		C.update(i, cities[i] = mPopulation[i]);
	}
	for(int i = 0;i < N - 1;i++){
...
bool exeed(int mid, int K, int mFrom, int mTo) {
	...
	for (int i = mFrom; i <= mTo; i++) {
		sum += cities[i];
		if (sum > mid) {
			sum = cities[i]; // mid값을 넘어가면 다른 그룹으로 됨. 
...
int divide(int mFrom, int mTo, int K){
	int start = cities[mFrom], end = C.query(mFrom, mTo);
	int ret = end;
	while (start <= end) {
		int mid = (start + end) / 2;
		if (exeed(mid, K, mFrom, mTo)) start = mid + 1; // 값이 클 때
		else {
			ret = mid; end = mid - 1; // 값이 작을 때
		}
	}
	return ret;
}
