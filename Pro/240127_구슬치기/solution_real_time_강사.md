...
struct Hole {
	...
	int operator-(const Hole& hole)const { // 두지점 사이의 이동비용 계산
		if (r == hole.r) return abs(c - hole.c) * 10; // 수평
		if (c == hole.c) return abs(r - hole.r) * 10; // 수직
		return abs(r - hole.r) * 14; // 대각
	}
...
int hitMarble(int mRow, int mCol, int mK) {
	...
	for (int i = 0; i < mK; i++) {
		...
		holeID = -1, holeDist = 1 << 20; // 구슬을 넣을 홀아이디와 거리 holeDist = 1048576
		search(hori[tg.r]);
		search(vert[tg.c]);
		if (leftRange[L].first <= R && R <= leftRange[L].second)
			search(leftRoad[L]);
